            ...
            {posts.map((post, index) => (
              ...
              >
                <div className="w-32 flex-shrink-0">
                  <span className="font-label text-xs tracking-widest text-on-surface-variant uppercase">{post.date}</span>
                </div>

                <div className="flex-grow">
                  <h2 className="text-2xl md:text-4xl font-semibold tracking-tight text-on-surface group-hover:text-surface-tint transition-colors">
                    <Link href={`/blog/${post.slug}`} className="inline-block relative">
                      {post.title}
                      <span className="absolute bottom-0 left-0 w-0 h-[2px] bg-[var(--accent)] transition-all duration-500 group-hover:w-full"></span>
                    </Link>
                  </h2>
                ...
                <div className="hidden md:block">
                  <span className="material-symbols-outlined text-on-surface-variant group-hover:text-surface-tint transition-transform duration-300 group-hover:translate-x-2">
                    arrow_forward
                  </span>
                </div>
              </article>
